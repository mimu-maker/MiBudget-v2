...
const DEFAULT_BUDGET_YEAR = new Date().getFullYear();
...
const fetchOrCreateDefaultBudget = async (profileId: string) => {
  // Try current year first, then any active budget
  ...
  // No budget found — return a synthetic stub so mutations can create one on demand
  return {
    id: 'fallback-id',
    user_id: profileId,
    year: DEFAULT_BUDGET_YEAR,
    name: `Budget ${DEFAULT_BUDGET_YEAR}`,
    budget_type: 'unified',

-- Migration for Annual Budget Configuration - Safe Version
-- Adds support for per-year budget configuration with sub-category visibility management

-- 1. Add year column to budgets table (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 2. Update the unique constraint to include year (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'budgets_user_id_name_budget_type_year_key'
    ) THEN
        -- Drop old constraint if it exists
        IF EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'budgets_user_id_name_budget_type_key'
        ) THEN
            ALTER TABLE public.budgets 
            DROP CONSTRAINT budgets_user_id_name_budget_type_key;
        END IF;
...
-- 3. Add budget_year column to transactions table (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 4. Create budget_sub_categories join table for per-year visibility
...
-- 5. Enable RLS for budget_sub_categories
ALTER TABLE public.budget_sub_categories ENABLE ROW LEVEL SECURITY;

-- 6. Create RLS policies for budget_sub_categories
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'budget_sub_categories' AND policyname = 'Users can view own budget sub-categories') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'budget_sub_categories' AND policyname = 'Users can manage own budget sub-categories') THEN
...
-- 7. Create function to auto-populate budget_year from transaction date
...
-- 8. Create trigger for auto-setting budget_year
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'set_transaction_budget_year_trigger' AND event_object_table = 'transactions') THEN
...
-- 9. Create function to auto-activate sub-category when used in a transaction
...
-- 10. Create trigger for auto-activating sub-categories
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'activate_sub_category_trigger' AND event_object_table = 'transactions') THEN
...
-- 11. Create function to get active sub-categories for a budget year
...
-- 12. Create function to initialize budget sub-categories from previous year
...
-- 14. Create trigger for updated_at on budget_sub_categories
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_budget_sub_categories_updated_at' AND event_object_table = 'budget_sub_categories') THEN
...
-- 15. Update existing budgets to have proper years (if they don't already)
UPDATE public.budgets 
SET year = EXTRACT(YEAR FROM start_date) 
WHERE year IS NULL OR year = 0;

-- 16. Update existing transactions to have budget_year (if they don't already)
